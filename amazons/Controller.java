...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("manual\\s+(\\w+)$", this::doManual),
        new Command("auto\\s+(\\w+)$", this::doAuto)
    };
    ...
    /** Moves from STR. */
    private void doMove(Matcher str) {
        Square from = sq(str.group(1));
        Square to = sq(str.group(3));
        Square spear = sq(str.group(5));
        _board.makeMove(from, to, spear);
        Piece winr = _board.winner();
        if (winr != null) {
            String ret;
            if (winr.toName().equals("White")) {
                ret = "White";
            } else if (winr.toName().equals("Black")) {
                ret = "Black";
            } else {
                ret = null;
            }
            _reporter.reportNote("%s wins.", ret);
        }
    }

    /** Set color from STR to auto. */
    private void doAuto(Matcher str) {
        String color = str.group(1);
    ...
    /** Set color from STR to manual. */
    private void doManual(Matcher str) {
        String color = str.group(1);
        String uppercolor = color.toUpperCase();
        if (uppercolor.equals("WHITE")) {
            _white = _manualPlayerTemplate.create(WHITE, this);
